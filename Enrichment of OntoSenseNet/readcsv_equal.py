...
mincount=min([Counter(y)[i] for i in Counter(y)])#take the minimum label frequency of all labels
count_arr=[0]*len(alllabels)

for i,vector in enumerate(X):
	if count_arr[y[i]]<mincount:
		count_arr[y[i]]=count_arr[y[i]]+1
		newTrainlabel.append(y[i])
		newTrain.append(vector)
	else:
		newTest.append(vector)
		newTestlabel.append(y[i])
...
test=newTest
train=newTrain
trainlabels=newTrainlabel
testlabels=newTestlabel
...
svmac=compare(output1,testlabels)
print(predictsion_recall_fscore_support(testlabels, output1, average='macro'))
...
#compare output and test_bianry_labels
rfac=compare(output3,testlabels)
...
#compare output and test_bianry_labels
nnac=compare(output4,testlabels)
